...
export function ManualCollectionPanel({ rows }: { rows: AdminManualCollectionRow[] }) {
  ...
  const record = useMutation({
    mutationFn: () => {
      if (!quote) throw new Error("That product and price combination is not in the price list.");
  ...
  const clientLink = useMutation({
    mutationFn: () => {
      if (!linkEntryId || !effectiveLinkClientId) throw new Error("Select a client account to link.");
  ...
  const siteLink = useMutation({
    mutationFn: () => {
      if (!linkEntryId || !effectiveLinkSiteId) throw new Error("Select a site to link.");
  ...
  const confirmCancellation = (row: AdminManualCollectionRow) => {
    const confirmed = window.confirm(
      `Cancel the ${MANUAL_COLLECTION_LABELS[row.productKind]} collection recorded for ${row.clientName}?
    ...
    <section className="mt-7" aria-labelledby="manual-collection-heading">
      <div className="mb-3">
        ...
        <p className="mt-0.5 text-[11px] leading-5 text-slate-500">
          You can record a collection before an account or a site exists. Entries and link history are never edited or deleted — a cancellation is recorded as a reversing entry.
        </p>
      </div>
      ...
      <Card className="p-4">
        ...
        >
          <fieldset className="lg:col-span-2 xl:col-span-4">
            <legend className="text-xs font-medium text-slate-600">How to record the customer</legend>
          ...
          {customerMode === 'manual' ? (
            <>
              <label className="text-xs font-medium text-slate-600">
                Customer name
                <input className={`${inputClass} mt-1`} value={customerName} onChange={(event) => setCustomerName(event.target.value)} maxLength={100} placeholder="e.g. the name on the Kmong order" />
              </label>
              <label className="text-xs font-medium text-slate-600">
                Contact note
              ...
              <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 lg:col-span-2">
                <p className="text-[11px] font-medium text-amber-800">Recorded without a linked account</p>
                <p className="mt-1 text-[11px] leading-5 text-amber-700">Link the account from the ledger row after they sign up, and the payment and its benefits are attributed then.</p>
              </div>
            </>
          ) : (
            <>
          ...
          {productKind === 'credit_pack' ? (
            ...
            <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
              <p className="text-[11px] text-slate-500">Amount from the price list</p>
          ...
          <label className="text-xs font-medium text-slate-600">
            ...
            <input className={`${inputClass} mt-1`} value={reference} onChange={(event) => setReference(event.target.value)} maxLength={160} placeholder="e.g. the Kmong order number" />
          </label>
          <label className="text-xs font-medium text-slate-600 lg:col-span-2">
            Memo
            <input className={`${inputClass} mt-1`} value={memo} onChange={(event) => setMemo(event.target.value)} maxLength={500} placeholder="Optional · client request or deposit confirmation" />
          </label>
          ...
          {record.isSuccess ? <p role="status" className="inline-flex items-center gap-1.5 text-xs text-emerald-700 xl:col-span-4"><CheckCircle2 size={13} aria-hidden />Recorded in the ledger.</p> : null}
        </form>
      </Card>
      ...
      <Card className="mt-2 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200 text-left text-xs">
            <thead className="bg-slate-50 text-slate-500"><tr><th className="px-3 py-2">Time</th><th className="px-3 py-2">Client / site</th><th className="px-3 py-2">Type</th><th className="px-3 py-2">Channel / reference</th><th className="px-3 py-2 text-right">Amount</th><th className="px-3 py-2 text-right">Actions</th></tr></thead>
                ...
                <Fragment key={row.entryId}>
                  <tr className={row.cancelled ? 'bg-slate-50/80' : undefined}>
                    ...
                    <td className="px-3 py-2">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">
                        {!row.clientId ? <Badge tone="amber">No account linked</Badge> : null}
                        {!row.siteId ? <Badge tone="neutral">Site not specified</Badge> : null}
                        {row.cancelled ? <Badge tone="red">Cancelled</Badge> : null}
                      </div>
                      {row.links.length ? <p className="mt-1 text-[10px] text-slate-400">{row.links.length} link entries</p> : null}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex min-w-28 flex-col items-end gap-1">
                        {!row.cancelled && !row.clientId ? <button type="button" onClick={() => openLink(row, 'client')} className="inline-flex items-center gap-1 rounded-md border border-blue-200 px-2 py-1 text-[11px] font-medium text-blue-700 hover:bg-blue-50"><Link2 size={11} aria-hidden />Link account</button> : null}
                        {!row.cancelled && row.clientId && !row.siteId ? <button type="button" onClick={() => openLink(row, 'site')} className="inline-flex items-center gap-1 rounded-md border border-slate-300 px-2 py-1 text-[11px] font-medium text-slate-600 hover:bg-slate-50"><Link2 size={11} aria-hidden />Link site</button> : null}
                  ...
                  {row.cancelled && row.reversal && showCancellationDetails ? (
                    <tr className="bg-red-50/40">
                      ...
                      <td className="px-3 py-2 text-red-700" colSpan={2}>Reversing entry · {row.reversal.memo ?? "Cancelled"}</td>
                      ...
                      <td className="px-3 py-2 text-right text-red-500">Original kept</td>
                    </tr>
                  ) : null}
                </Fragment>
      ...
      {linkingRow ? (
        <Card className="mt-3 border-blue-200 bg-blue-50/40 p-4">
          <h3 className="text-sm font-semibold text-blue-950">
            {linkMode === 'client' ? "Link a client account" : "Link a site"}
          </h3>
          <p className="mt-1 text-[11px] leading-5 text-blue-700">The original collection record is untouched; the link is added as a new row.</p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {linkMode === 'client' ? (
              ...
              <select className={inputClass} value={effectiveLinkSiteId} onChange={(event) => setLinkSiteId(event.target.value)}>
                {linkSites.length ? linkSites.map((site) => <option key={site.id} value={site.id}>{site.name}</option>) : <option value="">No sites available to link</option>}
              </select>
            )}
            <input className={inputClass} value={linkMemo} onChange={(event) => setLinkMemo(event.target.value)} maxLength={500} placeholder="Link note (optional)" />
          </div>
          <div className="mt-3 flex gap-2">
            <button type="button" onClick={() => (linkMode === 'client' ? clientLink.mutate() : siteLink.mutate())} disabled={linkMode === 'client' ? !effectiveLinkClientId || clientLink.isPending : !effectiveLinkSiteId || siteLink.isPending} className="rounded-md bg-blue-700 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">Add link</button>
