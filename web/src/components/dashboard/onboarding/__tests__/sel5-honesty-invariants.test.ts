...
describe('SEL5 — 애드온 데모 정직성·수명주기·비누출 불변식', () => {
  ...
  test('몰입 오버레이는 open 때만 lazy mount되고 unmount 시 소유한 listener·focus·scroll lock을 정리한다', () => {
    ...
    assert.match(SOURCES.immersive, /data-site-preview-scroll[\s\S]*const advance = \(time: number\)[\s\S]*scroller\.scrollTop = nextTop/);
    assert.match(SOURCES.immersive, /const stopAutoProgress = \(\) => \{[\s\S]*window\.cancelAnimationFrame\(frame\)/);
