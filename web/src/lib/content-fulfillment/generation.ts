...
function generationPrompt(input: {
  ...
  return [
    ...
    '- chart: {"type":"chart","kind":"bars"|"compare"|"steps","title":string,"unit"?:string,"items":[{"label":string,"value":number,"note"?:string,"sourceRef"?:string}],"caption"?:string,"sourceRefs":string[]}',
    ...
    'Charts are optional and rare. Emit a chart ONLY when the source material below already contains the numbers it would draw; otherwise write the article with no chart at all, which is the normal outcome.',
    'At most 2 chart blocks per article. 2–6 items each, and exactly 2 items when kind is "compare". Every value must be a non-negative number, and chart.sourceRefs must name at least one allowed source id.',
    'kind meanings: "bars" compares one measure across items; "compare" sets exactly two labelled values against each other; "steps" is an ordered sequence where each value is a duration or a count.',
    'Never invent, estimate, round up, extrapolate or infer a chart number. Every value must appear in the source material as that number. Put any qualification in the item note.',
