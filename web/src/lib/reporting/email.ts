...
/**
 * [CITE$] "Who got named in AI answers".
 *
 * Both renderers return '' when the section is absent, which is the normal case, so a
 * report without probes produces byte-for-byte the output it produced before this
 * feature existed. A test pins that.
 */
function aiAnswersHtml(section: ReportAiAnswersSection | undefined): string {
  if (!section) return '';
  const engineRows = section.engines.map((engine) => {
    const measured = engine.status === 'not_configured'
      ? '<span style="color:#6a7286">Not connected</span>'
      : `${NUMBER.format(engine.named)} · ${NUMBER.format(engine.linked)}`;
    return `<tr><td style="padding:7px 0;color:#232c52">${escapeHtml(engineName(engine.engine))}</td><td style="padding:7px 0;text-align:right;color:#141a3a;font-weight:650">${NUMBER.format(engine.asked)} asked · ${measured}</td></tr>`;
  }).join('');
  const questionRows = section.questions.map((question) => {
    const named = question.namedBy.length > 0
      ? `Named by ${escapeHtml(question.namedBy.map(engineName).join(', '))}`
      : 'Not named';
    const linked = question.linkedBy.length > 0
      ? ` · Linked by ${escapeHtml(question.linkedBy.map(engineName).join(', '))}`
      : '';
    return `<li style="margin:0 0 8px">${escapeHtml(question.question)}<div style="color:#6a7286;font-size:12px">${named}${linked}</div></li>`;
  }).join('');
  return `<h2 style="margin:26px 0 8px;font-size:17px">Who got named in AI answers</h2><table style="width:100%;border-collapse:collapse;font-size:14px">${engineRows}</table><ul style="margin:14px 0 0;padding-left:18px;font-size:13px;color:#232c52">${questionRows}</ul><p style="margin:12px 0 0;color:#6a7286;font-size:11px;line-height:1.6">${escapeHtml(section.footnote)}</p>`;
}
