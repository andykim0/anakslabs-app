...
export function buildMonthlyReportEmail(input: {
  ...
  const fourthHeadline = connectorMetrics
    ? metricCard('상담 행동', connectorMetrics.consultationActions)
    : metricCard('예약', metrics.reservationClicks);
  const supportingActions = connectorMetrics
    ? `카카오 상담 클릭 ${NUMBER.format(connectorMetrics.chatClicks.current)}건 · 문의 폼 제출 ${NUMBER.format(metrics.formSubmissions.current)}건 · 예약 클릭 ${NUMBER.format(metrics.reservationClicks.current)}건 · 인스타그램 클릭 ${NUMBER.format(connectorMetrics.instagramClicks.current)}건`
  ...
  const html = `<!doctype html><html lang="ko"><body style="margin:0;background:#f3f7fc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans KR',sans-serif;color:#071a33"><div style="max-width:680px;margin:0 auto;padding:32px 18px"><div style="border-radius:22px;background:#fff;padding:30px;box-shadow:0 12px 36px rgba(20,55,95,.08)"><div style="font-size:13px;font-weight:750;color:#1268e8">${PUBLIC_BRAND_NAMES.brand} 월간 성과 리포트</div><h1 style="margin:10px 0 4px;font-size:25px;line-height:1.35">${safeSiteName}의 ${monthLabel} 성과</h1><p style="margin:0;color:#65778b;font-size:13px">방문 수는 고유 방문자가 아니라 이 사이트에서 수집된 페이지 조회(페이지뷰) 기준입니다.</p>${comparisonNotice}<table role="presentation" style="width:100%;margin:20px -10px 0;border-collapse:separate"><tr>${metricCard('유입(페이지뷰)', metrics.pageviews)}${metricCard('전화', metrics.phoneClicks)}${metricCard('길찾기', metrics.directionsClicks)}${fourthHeadline}</tr></table><div style="margin-top:20px;border-radius:14px;background:#ecf6ff;padding:17px 18px;font-size:16px;font-weight:700;color:#0a477f">${escapeHtml(input.report.insight)}</div><h2 style="margin:26px 0 8px;font-size:17px">유입 출처 구성</h2><table style="width:100%;border-collapse:collapse;font-size:14px">${sourceRows}</table><p style="margin:20px 0 0;color:#52657a;font-size:13px">${escapeHtml(supportingActions)}</p><div style="margin-top:26px">${dashboardLink}</div></div><p style="margin:16px 6px 0;color:#7d8c9d;font-size:11px;line-height:1.6">이 리포트는 다보임 사이트에서 수집된 익명 집계 수치만으로 생성됐습니다. 검색 순위나 성과를 보장하지 않습니다.</p></div></body></html>`;
  ...
  const text = [
    ...
    ...(connectorMetrics
      ? [
          `상담 행동: ${NUMBER.format(connectorMetrics.consultationActions.current)}건 · ${comparisonLabel(connectorMetrics.consultationActions)}`,
          supportingActions,
        ]
      : [
