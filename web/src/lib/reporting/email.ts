...
export function buildMonthlyReportEmail(input: {
  ...
  const sourceRows = activeSources.length > 0
    ? activeSources.map((source) => `<tr><td style="padding:7px 0;color:#273c53">${escapeHtml(source.label)}</td><td style="padding:7px 0;text-align:right;color:#071a33;font-weight:650">${NUMBER.format(source.count)} · ${source.sharePercent}%</td></tr>`).join('')
    : '<tr><td style="padding:7px 0;color:#65778b">No traffic has been measured yet.</td></tr>';
  const dashboardLink = dashboardUrl
    ? `<a href="${escapeHtml(dashboardUrl)}" style="display:inline-block;padding:12px 18px;border-radius:999px;background:#1268e8;color:#fff;text-decoration:none;font-weight:700">View the full report</a>`
    : '';
  const comparisonNotice = input.report.hasComparisonData
    ? ''
    : '<div style="margin-top:18px;border:1px solid #dbe7f5;border-radius:12px;padding:13px 15px;color:#52657a;font-size:13px">This is the first report. Starting this month, we will collect data for month-over-month comparisons.</div>';
  ...
  const html = `<!doctype html><html lang="en"><body style="margin:0;background:#f3f7fc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#071a33"><div style="max-width:680px;margin:0 auto;padding:32px 18px"><div style="border-radius:22px;background:#fff;padding:30px;box-shadow:0 12px 36px rgba(20,55,95,.08)"><div style="font-size:13px;font-weight:750;color:#1268e8">${PUBLIC_BRAND_NAMES.brand} monthly performance report</div><h1 style="margin:10px 0 4px;font-size:25px;line-height:1.35">${safeSiteName} — ${monthLabel}</h1><p style="margin:0;color:#65778b;font-size:13px">Visits are measured as page views, not unique visitors.</p>${comparisonNotice}<table role="presentation" style="width:100%;margin:20px -10px 0;border-collapse:separate"><tr>${metricCard('Page views', metrics.pageviews)}${metricCard('Calls', metrics.phoneClicks)}${metricCard('Directions', metrics.directionsClicks)}${fourthHeadline}</tr></table><div style="margin-top:20px;border-radius:14px;background:#ecf6ff;padding:17px 18px;font-size:16px;font-weight:700;color:#0a477f">${escapeHtml(input.report.insight)}</div><h2 style="margin:26px 0 8px;font-size:17px">Traffic sources</h2><table style="width:100%;border-collapse:collapse;font-size:14px">${sourceRows}</table><p style="margin:20px 0 0;color:#52657a;font-size:13px">${escapeHtml(supportingActions)}</p><div style="margin-top:26px">${dashboardLink}</div></div><p style="margin:16px 6px 0;color:#7d8c9d;font-size:11px;line-height:1.6">This report uses only anonymous aggregate measurements collected for the website. It does not guarantee search rankings or business results.</p></div></body></html>`;
