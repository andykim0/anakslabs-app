-- USD-PAYMENTS: bind Stripe's recurring subscription identity to the existing
-- authoritative site_subscriptions row. Legacy/manual subscriptions remain
-- valid with a null provider identity.

alter table public.site_subscriptions
  add column stripe_subscription_id text;

alter table public.site_subscriptions
  add constraint site_subscriptions_stripe_id_check check (
    stripe_subscription_id is null
    or length(btrim(stripe_subscription_id)) > 0
  );

create unique index site_subscriptions_stripe_id_idx
  on public.site_subscriptions (stripe_subscription_id)
  where stripe_subscription_id is not null;

comment on column public.site_subscriptions.stripe_subscription_id is
  'Stripe subscription identity used to resolve signed invoice.paid renewals. Null preserves mock, manual, and historical subscriptions.';

-- USD recurring receipts use a dedicated transaction boundary. It validates
-- the server-pinned site contract, records whole dollars, extends the existing
-- authoritative subscription, and binds the Stripe ID without invoking any
-- credit grant or tier mutation used by the historical KRW functions.
...
begin
  ...
  if p_amount is null
    ...
    raise exception 'handle_usd_industry_maintenance_payment: amount must equal 990 whole USD'
      using errcode = '23514';
  end if;
  ...
  -- Serialize every renewal and provider-binding decision for this owner. The
  -- nested renew_site_subscription call takes the same transaction-level lock.
  ...
  if not found then
    -- A concurrent delivery won the unique-key race. Re-read and require the
    -- same effect; its transaction also owns the subscription extension.
    select * into v_payment
    from public.payments p
    where p.provider_payment_key = p_provider_payment_key;
    if not found
