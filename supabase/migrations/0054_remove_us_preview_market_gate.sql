-- SiteMeta.market carried no information: every US writer emitted the same unexplained
-- literal alongside locale and jurisdiction. Keep the security-definer preview gate
-- fail-closed on the two meaningful server-authored fields while removing only market.
