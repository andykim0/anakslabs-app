-- Record the real preview page slug and retain US outreach telemetry for the full sequence.
-- Existing rows are preserved; the previous UUID-shaped slug values remain format-valid.

alter table public.demo_views
  drop constraint demo_views_slug_check;

alter table public.demo_views
  add constraint demo_views_slug_check check (
    char_length(slug) <= 40
    and (
      slug = ''
...
alter table public.demo_view_alerts
  add constraint demo_view_alerts_signal_kind_check check (
    signal_kind in ('strong_reinterest_48h', 'procedure_entry')
  );

alter table public.demo_view_alerts
  drop constraint demo_view_alerts_visit_count_check;

alter table public.demo_view_alerts
  add constraint demo_view_alerts_visit_count_check check (visit_count >= 1);

alter table public.demo_view_alerts
  alter column hours_since_last drop not null;

alter table public.demo_view_alerts
  drop constraint demo_view_alerts_hours_since_last_check;

alter table public.demo_view_alerts
  add constraint demo_view_alerts_hours_since_last_check check (
    hours_since_last is null
    or hours_since_last between 0 and 48
  );

comment on table public.demo_views is
  'Ninety-day service-only page-level heartbeat ledger for expiring US medical outreach previews.';

comment on table public.shared_site_previews is
  'Bearer previews retain standard imports for fourteen days and US medical outreach previews for forty-five days; only a SHA-256 token hash is stored.';
