...
() => {
  ...
  if (secs.length <= 2 && secs.length > 0) {
    const inner = [];
    for (const s of secs) { let added = 0;
      for (const ch of s.children) { if (!vis(ch)) continue;
        if (ch.getBoundingClientRect().height > 180) { inner.push(ch); added++; } }
      if (!added) inner.push(s); }
  ...
  if (hero) {
    ...
    const ctaRe = /book|appointment|schedule|consult|request|contact|call|get started|new patient|reserve/i;
    const ctas = [];
  ...
  out.features = {
    ...
    kwReviews: has(/testimonial|patient stories|what our patients|reviews?\b|rated \d|\d\.\d\s*(stars?|out of)/i),
    kwFaq: has(/frequently asked|FAQ\b|common questions/i),
    kwInsurance: has(/insurance|financing|payment plan|membership plan|CareCredit|in-network/i),
    kwPromo: has(/special offer|new patient special|promotion|limited time|save \$|free consultation|complimentary consultation/i),
    kwBeforeAfter: has(/before\s*(&|and|\/)\s*after|smile gallery|smile transformation|case gallery|our results|our work/i),
    kwTeam: has(/meet (the |our )?(team|dr\.?|doctor)|our (doctors|dentists|team)|about (dr\.?|the doctor)/i),
    kwImplant: has(/implant/i), kwCosmetic: has(/cosmetic|veneer|smile makeover|esthetic|aesthetic/i),
    kwUSState: (pageText.match(/\b(AL|AK|AZ|AR|CA|CO|CT|DE|FL|GA|HI|ID|IL|IN|IA|KS|KY|LA|ME|MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|OH|OK|OR|PA|RI|SC|SD|TN|TX|UT|VT|VA|WA|WV|WI|WY|DC)\s+\d{5}(-\d{4})?/) || [null])[0],
    phoneUS: (pageText.match(/\(?\d{3}\)?[\s.-]\d{3}[\s.-]\d{4}/) || [null])[0],
    statLike: [...new Set((pageText.match(/\b\d{1,3}(,\d{3})*\+?\s*(years?|yrs|patients|implants|smiles|cases|procedures|reviews|5[- ]star)/gi)||[]))].slice(0,12) };
  return out;
}
"""
