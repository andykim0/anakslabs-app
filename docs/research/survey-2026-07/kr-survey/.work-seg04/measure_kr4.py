...
() => {
  ...
  out.features = {
    ...
    kwReviews: has(/후기|리뷰|고객\s*이야기|환자\s*이야기|치료\s*후기|시술\s*후기|만족도/),
    kwFaq: has(/자주\s*묻는|FAQ|자주하는\s*질문|궁금|Q\s*&\s*A|묻고\s*답하기/i),
    kwInsurance: has(/실비|실손|보험|비용|가격|진료비|비급여|할부|카드/),
    kwPromo: has(/이벤트|프로모션|할인|특가|한정|무료\s*상담|혜택/),
    kwBeforeAfter: has(/전후|비포\s*애프터|before\s*(&|and|\/)\s*after|시술\s*전후|변화\s*사진/i),
    kwTeam: has(/의료진|원장|전문의|대표원장|의사\s*소개|닥터/),
    kwBooking: has(/예약|상담\s*신청|온라인\s*상담|비대면|카카오|카톡|전화\s*상담|문의/),
    kwLocation: has(/오시는\s*길|찾아오시는|위치|주소|진료시간|약도/),
    kwGallery: has(/둘러보기|내부\s*전경|시설|갤러리|인테리어|공간\s*소개/),
    kwTrust: has(/인증|학회|논문|특허|정회원|수상|누적|보건복지부|의료기관\s*인증/),
    kwVideo: has(/영상|유튜브|YouTube|동영상/i),
    phoneKR: (pageText.match(/0\d{1,2}[-\s.]\d{3,4}[-\s.]\d{4}/) || [null])[0],
